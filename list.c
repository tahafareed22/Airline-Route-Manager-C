 ...
 * @param repitition The number of times the airline appears
 *
 * @return node_t* A pointer to the node created.
 *
 */
node_t *new_node(char* name, int repitition)
{
    assert(repitition != 0 && name!= NULL);

    node_t *temp = (node_t *)emalloc(sizeof(node_t));

    temp->subject = strdup(name);
    temp -> statistic = repitition;
    temp->next = NULL;
...
node_t *add_inorder_descending(node_t *list, node_t *new) {
    node_t *prev = NULL;
    node_t *curr = NULL;
...
node_t *find_in_list(node_t *list, char *searched){
    ...
    for (curr = list; curr != NULL; curr = curr->next) {
        if (strcmp(searched, curr->subject) == 0) {
            return curr;
        }
    }   
